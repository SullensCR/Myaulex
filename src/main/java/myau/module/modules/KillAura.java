...
public class KillAura extends Module {
    ...
    private long getAttackDelay() {
        return this.isBlocking
                ? 1000L / this.autoBlockTargetCPS.getValue()
    ...
    public KillAura() {
        ...
        this.autoBlockTargetCPS = new IntProperty("auto-block-target-cps", 10, 1, 10);
        this.autoBlockTargetCPS.setDisplayName("Target CPS");
        this.autoBlockRange = new FloatProperty("auto-block-range", 6.0F, 3.0F, 8.0F);
        this.swingRange = new FloatProperty("swing-range", 3.5F, 3.0F, 6.0F);
        this.attackRange = new FloatProperty("attack-range", 3.0F, 3.0F, 6.0F);
        this.fov = new IntProperty("fov", 360, 30, 360);
        this.targetCPS = new IntProperty("target-cps", 10, 1, 20);
        this.targetCPS.setDisplayName("Target CPS");
    ...
    /**
     * Returns the closest target that Aura would accept for its visual HUD
     * range. This deliberately does not mutate Aura's combat target: Aura
     * itself continues to select and attack only from its normal swing/attack
     * ranges.
     */
    public EntityLivingBase getHudTarget() {
        if (!this.isEnabled() || !this.isAttackAllowed() || mc.theWorld == null) {
            return null;
        }
        ...
        for (Entity entity : mc.theWorld.loadedEntityList) {
            ...
            if (!this.isValidTarget(candidate) || !this.isInBlockRange(candidate)) {
                continue;
            }

            double distance = RotationUtil.distanceToEntity(candidate);
    ...
    public void verifyValue(String value) {
        if (!this.autoBlock.getName().equals(value)) {
            ...
            } else if (this.autoBlockTargetCPS.getName().equals(value)
                    && this.autoBlockTargetCPS.getValue() > this.autoBlockTargetCPS.getMaximum()) {
                this.autoBlockTargetCPS.setValue(this.autoBlockTargetCPS.getMaximum());
            }
        }
    }
